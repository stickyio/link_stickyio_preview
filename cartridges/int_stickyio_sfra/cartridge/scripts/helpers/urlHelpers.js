 ...
 * The original function will not work with multiple options because it sees "options" already exists as a key, and therefore skips all additional option values
 *
    ...
    if (!Array.isArray(queryParamsToAppend) && typeof queryParamsToAppend === 'object') {
        ...
        targetQueryString = queryParamsToAppend.reduce(function (current, params) {
            ...
            Object.keys(queryToAppend).forEach(function (key) {
                if (key === 'options') {
                    if (!currentCopy[key]) { currentCopy[key] = []; }
                    currentCopy[key].push(queryToAppend.options[0]);
                } else if (!Object.prototype.hasOwnProperty.call(currentCopy, key)) {
